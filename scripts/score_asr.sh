#!/bin/bash
# ============================================================
# Stage 5: ASR intelligibility (WER / CER) with Whisper-small (SLURM, 1 GPU).
...
#
# Predictions:  WAV_DIRS="/path/a /path/b" sbatch scripts/score_asr.sh
# Baseline:     ROLE=healthy OUT_DIR=/some/dir sbatch scripts/score_asr.sh
#               ROLE=source  OUT_DIR=/some/dir sbatch scripts/score_asr.sh
# Override:     MANIFEST=/path.tsv TRANSCRIPTS=/path.csv OVERWRITE=true ...
# ============================================================
#SBATCH --job-name=melvc_asr
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=16
#SBATCH -p zen4_0768_h100x4
#SBATCH --qos zen4_0768_h100x4
#SBATCH --time=2:00:00
#SBATCH --output=/tmp/melvc_asr_%j.log
#SBATCH --error=/tmp/melvc_asr_%j.log
...
exec >> "$ROOT/exp/eval-logs/melvc_asr_${SLURM_JOBID}.log" 2>&1
...
ROLE=${ROLE:-pred}
MODEL=${MODEL:-openai/whisper-small}      # e.g. openai/whisper-medium, openai/whisper-large-v3
...
echo "[asr] role=$ROLE  model=$MODEL  manifest=$MANIFEST  transcripts=$TRANSCRIPTS"

if [ "$ROLE" = "pred" ]; then
    : "${WAV_DIRS:?set WAV_DIRS=\"/dir1 /dir2 ...\" for role=pred}"
    python scripts/score_asr.py $WAV_DIRS \
