#!/bin/bash
#
# Multi-Node End-to-End Speech Synthesis Training: Stage 1 + HiFi-GAN
#
# This is the multi-node version of fineTuneSpeechNoLLM_E2E.sh.
# All training hyperparameters are identical to the single-node version.
# The only additions are SLURM-aware distributed training configuration.
#
# Usage: launched via submit_multinode.sh (sbatch), NOT directly.
#
...
# Compute global base rank for this node (fairseq adds local GPU index internally)
# Node 0 -> start_rank=0 (spawns 0,1), Node 1 -> start_rank=2 (spawns 2,3), etc.
...
OUT_PATH=$ROOT/exp/mms-speech-NoLLM-E2E/$RUN_NAME

# Pretrained checkpoints (adjust paths for this cluster)
...
echo "====== Multi-Node Training Config ======"
...
if [ "${RESUME}" = "true" ]; then
    ...
    # Read saved overrides, strip "- " prefix, drop distributed_training.* and hydra.run.dir
