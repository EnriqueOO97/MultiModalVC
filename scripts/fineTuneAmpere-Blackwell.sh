...
NGPUS=1
QPS=3 
OUT_PATH=$ROOT/exp/mms-llama/german_finetune1GPU-96gb-wtf
...
# Accumulate gradients (update_freq=8) to simulate larger batch size on 1 GPU
...
CUDA_VISIBLE_DEVICES=0 fairseq-hydra-train \
    ...
    dataset.batch_size=70 \
    dataset.max_tokens=10000 \
    dataset.required_batch_size_multiple=2 \
    ...
    model.w2v_path=$ROOT/pretrained_models/avhubert/muavic_multilingual_compatible.pt \
